...
function projectSelectionProjection(row) {
  return {
    id: row.id,
    ...
    projects: projectRows.filter(
      (project) => project.selected_repository_id === row.id,
    ),
  };
}
...
function createFixtureServer() {
  return createServer(async (request, response) => {
    ...
    if (
      ...
      writeJson(
        ...
        select.includes("projects(")
          ? selectedRows.map(projectSelectionProjection)
          : selectedRows.map(selectionProjection),
      );
      return;
    }
    ...
    if (
      request.method === "POST" &&
      requestUrl.pathname === "/rest/v1/rpc/save_project_calibration"
    ) {
      const body = JSON.parse(await readBody(request));
      const repository = selectedRows.find(
        (row) => row.id === body.p_selected_repository_id,
      );
      if (!repository || body.p_user_id !== authUserId) {
        writeJson(response, 400, {
          message: "project_calibration_selected_repository_not_found",
        });
        return;
      }
      const existing = projectRows.find(
        (project) =>
          project.selected_repository_id === repository.id &&
      ...
      projectRows = [
        ...projectRows.filter((project) => project.id !== row.id),
        row,
      ];
      projectWriteCalls += 1;
      writeJson(response, 200, {
        ...
        selected_repositories: {
          ...projectSelectionProjection(repository),
          projects: [],
        },
      });
      return;
    ...
    if (
      ...
      writeJson(response, 200, {
        ...
        project_read_calls: projectReadCalls,
        ...
        project_created: projectRows.length > 0,
        project_write_calls: projectWriteCalls,
        project_count: projectRows.length,
        active_project_count: projectRows.filter(
    ...
    [
      ...
      `--config=${
        process.env.PROJECT_CALIBRATION_FIXTURE === "1"
          ? "playwright.project-calibration-fixture.config.ts"
          : "playwright.repository-selection-fixture.config.ts"
      }`,
    ],
    {
      cwd: projectRoot,
