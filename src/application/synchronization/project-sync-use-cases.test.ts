...
function fixture(triggerSource: "webhook" | "reconciliation" | "manual" = "manual", options: { status?: SyncRun["status"]; installationStatus?: "active" | "suspended" | "revoked"; revokeAfterCommitRead?: boolean; writerFailure?: string; tokenFailure?: string; commits?: GitHubCommitReadModel[] } = {}) {
  ...
  const contexts = { getByProjectId: vi.fn(async () => options.revokeAfterCommitRead && reader.listCommits.mock.calls.length > 0 ? { ...context, installation: { ...context.installation, status: "revoked" as const } } : context) };
  const useCase = new ExecuteProjectSynchronization({ runs, contexts, tokens: { issue: vi.fn(async () => { if (options.tokenFailure) throw new Error(options.tokenFailure); return { token: "synthetic-token", expiresAt: at }; }) }, reader, metadata: { read: vi.fn(async () => ({ githubObjectId: "91001", repositoryFullName: "synthetic/repository", sourceUpdatedAt: at, sourceVersion: "c".repeat(64), defaultBranch: "main", visibility: "private" as const, isPrivate: true, isFork: false, isArchived: false, isDisabled: false })) }, writer, clock: { now: () => new Date(at) } });
...
describe("project-synchronization-runtime.v1", () => {
  ...
  it("drops a GitHub result when authorization is revoked before persistence", async () => { const f = fixture("manual", { revokeAfterCommitRead: true }); await expect(f.useCase.execute({ job: job() })).resolves.toMatchObject({ status: "failed", authorizationRevoked: true }); expect(f.writer.upsertGroup).toHaveBeenCalledTimes(1); expect(f.writer.upsertGroup).not.toHaveBeenCalledWith(expect.objectContaining({ groupName: "commit" })); expect(f.reader.listIssues).not.toHaveBeenCalled(); });
