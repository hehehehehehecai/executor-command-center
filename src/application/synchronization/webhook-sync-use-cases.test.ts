...
function fixture(status: "dispatched" | "processing" | "completed" = "dispatched") {
  ...
  const requests = { request: vi.fn(async () => ({ outcome: "new" as const, projectId, syncRunId: runId, syncRunStatus: "queued" as const, dispatchState: "pending" as const, dispatchVersion: 1 })), claimDispatch: vi.fn(async () => ({ claimed: true, version: 2 })), completeDispatch: vi.fn(async () => undefined) };
