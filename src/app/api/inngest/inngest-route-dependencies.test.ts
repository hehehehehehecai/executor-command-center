...
describe("inngest-route-composition.v1", () => {
  ...
  it("composes one client and five real functions without network at construction", () => { const fetchSpy = vi.spyOn(globalThis, "fetch"); const result = createInngestRouteDependencies(valid); expect(inngestRouteCompositionContract).toBe("inngest-route-composition.v1"); expect(result.functions).toHaveLength(5); expect(fetchSpy).not.toHaveBeenCalled(); fetchSpy.mockRestore(); });
});
