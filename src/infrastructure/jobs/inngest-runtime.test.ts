...
function fixture() { const firstSync = { execute: vi.fn(async () => "first") }; const projectSync = { execute: vi.fn(async () => "project") }; const webhooks = { request: vi.fn(async () => "webhook-request"), execute: vi.fn(async () => "webhook-execute") }; const daily = { handle: vi.fn(async () => "daily") }; const accountDeletion = { execute: vi.fn(async () => "deleted"), markRetryExhausted: vi.fn(async () => "retry-eligible"), recover: vi.fn(async () => ({ eligible: 0, dispatched: 0, failed: 0 })) }; return { dependencies: { firstSync, projectSync, webhooks, daily, accountDeletion }, firstSync, projectSync, webhooks, daily, accountDeletion }; }
...
describe("inngest-synchronization-runtime.v1", () => {
  it("registers synchronization, daily reconciliation, due deletion and bounded recovery functions", () => { const f = fixture(); const functions = createInngestSynchronizationFunctions(new Inngest({ id: "synthetic-runtime", eventKey: "synthetic-event-key", signingKey: "signkey-test-synthetic" }), f.dependencies); expect(inngestSynchronizationRuntimeContract).toBe("inngest-synchronization-runtime.v1"); expect(functions).toHaveLength(5); expect(inngestSynchronizationFunctionDefinitions.map((item) => item.id)).toEqual(["executor-project-sync-consumer", "executor-github-webhook-consumer", "executor-daily-reconciliation", "executor-account-deletion", "executor-account-deletion-recovery"]); expect(inngestSynchronizationFunctionDefinitions[2]).toMatchObject({ trigger: "cron:0 2 * * *", idempotency: "domain:reconciliation:<UTC-day>" }); });
  it("routes a strict due account deletion job by operation id and generation", async () => { const f = fixture(); const handlers = createInngestSynchronizationHandlers(f.dependencies); await expect(handlers.accountDeletion({ version: "account-deletion-job.v1", jobType: "account.deletion.due.v1", jobId: `${runId}:0`, operationId: runId, generation: 0, dueAt: at })).resolves.toBe("deleted"); expect(f.accountDeletion.execute).toHaveBeenCalledWith({ operationId: runId }); });
  it("marks retry exhaustion when storage fails before claim and runs bounded recovery separately", async () => { const f = fixture(); const handlers = createInngestSynchronizationHandlers(f.dependencies); const deletionJob = { version: "account-deletion-job.v1", jobType: "account.deletion.due.v1", jobId: `${runId}:3`, operationId: runId, generation: 3, dueAt: at }; f.accountDeletion.execute.mockRejectedValueOnce(new Error("account_deletion_storage_failed")); await expect(handlers.accountDeletion(deletionJob)).rejects.toThrow("account_deletion_storage_failed"); await expect(handlers.accountDeletionFailure(deletionJob)).resolves.toBe("retry-eligible"); expect(f.accountDeletion.markRetryExhausted).toHaveBeenCalledWith({ operationId: runId, generation: 3 }); await expect(handlers.accountDeletionRecovery()).resolves.toEqual({ eligible: 0, dispatched: 0, failed: 0 }); });
