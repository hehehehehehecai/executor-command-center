...
function fixture() { const firstSync = { execute: vi.fn(async () => "first") }; const projectSync = { execute: vi.fn(async () => "project") }; const webhooks = { request: vi.fn(async () => "webhook-request"), execute: vi.fn(async () => "webhook-execute") }; const daily = { handle: vi.fn(async () => "daily") }; const accountDeletion = { execute: vi.fn(async () => "deleted") }; return { dependencies: { firstSync, projectSync, webhooks, daily, accountDeletion }, firstSync, projectSync, webhooks, daily, accountDeletion }; }
...
describe("inngest-synchronization-runtime.v1", () => {
  it("registers synchronization, daily reconciliation and due account deletion functions", () => { const f = fixture(); const functions = createInngestSynchronizationFunctions(new Inngest({ id: "synthetic-runtime", eventKey: "synthetic-event-key", signingKey: "signkey-test-synthetic" }), f.dependencies); expect(inngestSynchronizationRuntimeContract).toBe("inngest-synchronization-runtime.v1"); expect(functions).toHaveLength(4); expect(inngestSynchronizationFunctionDefinitions.map((item) => item.id)).toEqual(["executor-project-sync-consumer", "executor-github-webhook-consumer", "executor-daily-reconciliation", "executor-account-deletion"]); expect(inngestSynchronizationFunctionDefinitions[2]).toMatchObject({ trigger: "cron:0 2 * * *", idempotency: "domain:reconciliation:<UTC-day>" }); });
  it("routes a strict due account deletion job by operation id", async () => { const f = fixture(); const handlers = createInngestSynchronizationHandlers(f.dependencies); await expect(handlers.accountDeletion({ version: "account-deletion-job.v1", jobType: "account.deletion.due.v1", jobId: runId, operationId: runId, dueAt: at })).resolves.toBe("deleted"); expect(f.accountDeletion.execute).toHaveBeenCalledWith({ operationId: runId }); });
