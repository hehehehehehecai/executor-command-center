...
describe("inngest-synchronization-runtime.v1", () => {
  ...
  it("marks retry exhaustion from the original failed job and runs bounded recovery separately", async () => { const f = fixture(); const handlers = createInngestSynchronizationHandlers(f.dependencies); const job = { version: "account-deletion-job.v1", jobType: "account.deletion.due.v1", jobId: `${runId}:3`, operationId: runId, generation: 3, dueAt: at }; await expect(handlers.accountDeletionFailure(job)).resolves.toBe("retry-eligible"); expect(f.accountDeletion.markRetryExhausted).toHaveBeenCalledWith({ operationId: runId, generation: 3 }); await expect(handlers.accountDeletionRecovery()).resolves.toEqual({ eligible: 0, dispatched: 0, failed: 0 }); });
