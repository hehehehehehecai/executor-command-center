...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `/` | Preview、同步中 | 唯一顶级 banner、桌面主导航、`main#main-content` 与 H1；banner 位于 main 外 | Skip link；移动导航触发、焦点循环、Escape、恢复触发器 | 同步状态有文字标签，不只依赖颜色 | 四档无横向溢出 | `A11Y-LANDMARK-01`、`A11Y-DRAWER-01`、`A11Y-RESPONSIVE-01` | 375 抽屉遮罩、层级与阅读顺序通过 |
| `/onboarding` | 未登录、未连接、账户删除确认 | 唯一 main/H1 | 确认输入初始聚焦；Tab/Shift+Tab 循环；背景键盘/指针不可达；Escape 恢复准确触发器；pending 禁止关闭且清理无残留 | 账户状态、七天窗口、失败保持文字说明 | 320 身份指标改为单列，无截断 | `A11Y-CONFIRM-02`、`A11Y-LANDMARK-01`、`A11Y-RESPONSIVE-01`、`AccountDeletionPanel.test.tsx` | 320 单列指标与动作阅读顺序通过 |
...
| `/project-galaxy` | Preview、Connected、失败、两种移除确认 | Preview/Connected/错误分支均唯一 main/H1；Connected 生命周期和移除动作均包含于 `main#main-content` | 确认输入初始聚焦；Tab/Shift+Tab 循环；背景键盘/指针不可达；Escape 恢复对应模式触发器 | 正式状态/建议状态文字区分；失败有安全返回路径 | 四档无横向溢出，高风险区不被挤出 | `A11Y-CONFIRM-01`、`A11Y-LANDMARK-01`、`A11Y-STATE-01`、`A11Y-RESPONSIVE-01` | 1280 正式事实、建议、数据新鲜度层级通过 |
...
| 目标 | 分母 | 通过标准 | 证据 |
| --- | ---: | --- | --- |
| Landmark / heading | 8 个主路由 + 1 个 Connected containment | 每页唯一 main、唯一 H1、标题层级不跳级；`/` 有唯一顶级 banner 与桌面主导航；Connected 核心操作位于 main；skip link 首个 Tab 可见且将焦点送入 main | `A11Y-LANDMARK-01`，9/9 对账 |
| 移动导航 | 1 个独立交互 case | dialog/aria-modal/可理解名称；打开聚焦关闭按钮；Tab 循环；Escape 与遮罩关闭；焦点恢复 | `A11Y-DRAWER-01`，1/1 |
| 强确认 | 2 个独立组件、2 个浏览器 case | 打开后进入确认输入；Tab/Shift+Tab 双向循环；背景设置 `inert`/`aria-hidden` 且可信指针无副作用；pending 不允许 Escape；关闭/成功后 body、inert、监听器清理并恢复准确触发器 | 浏览器 `A11Y-CONFIRM-01`、`A11Y-CONFIRM-02`，2/2；聚焦单测 42/42 |
...
- Phase 6.1 红灯：聚焦单测 4 个目标失败、33 个既有断言通过；真实浏览器 2 个目标失败、6 个既有 case 通过，分别命中 banner 缺失、Connected 操作不在 main 与确认焦点未循环。
- Phase 6.1 最终专用套件：9/9 通过，保留原 8 个 case 与 32/32 route/viewport 组合；新增 Account Deletion 合成本地 Auth case；skip 0，console error 0，page error 0，未处理请求 0，非本地请求 0。
